...
export const Admin: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    else if (!currentUser.isAdmin) navigate('/');
    ...
    <div className="w-full max-w-7xl mx-auto flex flex-col items-center">
      <div className="w-full max-w-5xl">
          <div className="mb-10 text-center border-b border-[#262626] pb-8">
            <h1 className="text-3xl font-serif tracking-tight text-white uppercase mt-4">Painel Administrativo</h1>
            <h3 className="mt-4 text-[10px] font-bold uppercase tracking-widest text-[#D4AF37] flex items-center justify-center gap-2">
          ...
          {orders.length === 0 ? (
            <div className="text-center py-20 bg-[#0E0E0E] border border-[#262626]">
              ...
              <p className="text-[10px] uppercase tracking-widest text-[#A1A1AA]">Sistema aguardando transações.</p>
            </div>
          ) : (
            <div className="bg-[#070707] border border-[#262626] overflow-hidden flex flex-col">
              <div className="bg-[#151515] p-6 text-[9px] uppercase tracking-widest font-bold border-b border-[#262626] grid grid-cols-4 sm:grid-cols-5 text-[#A1A1AA] gap-4">
                 ...
                 <span className="col-span-2">Cliente / Detalhes</span>
                ...
                {orders.map((order) => (
                  <div key={order.id} className="p-6 grid grid-cols-4 sm:grid-cols-5 items-center hover:bg-[#0A0A0A] transition-colors gap-4">
                    ...
                    <div className="col-span-2 space-y-1">
                      <p className="text-xs text-white uppercase tracking-wider">{order.customer.name}</p>
                      <p className="text-[9px] text-[#A1A1AA] uppercase tracking-widest mt-1 mb-2 block">{order.paymentMethod} • {order.items.length} ITENS</p>
          ...
          <p className="mt-8 text-[9px] uppercase tracking-widest text-[#555] text-center">
             SISTEMA SINCRONIZADO VIA STORE. DADOS VOLÁTEIS.
          </p>
      </div>
    </div>
  );
